...
CREATE TABLE IF NOT EXISTS `mydb`.`SportClubs` (
  ...
  INDEX `fk_SportComplex_Sotrudniki1_idx` (`idEmployees` ASC) VISIBLE,
  INDEX `fk_SportComplex_Event1_idx` (`idEvents` ASC) VISIBLE,
  INDEX `fk_SportComplex_Partner1_idx` (`idSponsors` ASC) VISIBLE,
  UNIQUE INDEX `addres_UNIQUE` (`addres` ASC) VISIBLE,
  UNIQUE INDEX `number_UNIQUE` (`number` ASC) VISIBLE,
  UNIQUE INDEX `mail_UNIQUE` (`mail` ASC) VISIBLE,
  CONSTRAINT `fk_SportComplex_Sotrudniki1`
    FOREIGN KEY (`idEmployees`)
    REFERENCES `mydb`.`Employees` (`idEmployees`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_SportComplex_Event1`
    FOREIGN KEY (`idEvents`)
    REFERENCES `mydb`.`Events` (`idEvents`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_SportComplex_Partner1`
    FOREIGN KEY (`idSponsors`)
    REFERENCES `mydb`.`Sponsors` (`idSponsors`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
